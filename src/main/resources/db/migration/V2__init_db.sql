...
(
    ...
    timezone varchar(65) DEFAULT 'Europe/Madrid' NOT NULL,
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_appointment_times'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_service_duration'
    ) THEN
        ALTER TABLE service
...
CREATE INDEX IF NOT EXISTS idx_appointment_therapist_status ON appointment (therapist_id, status_code);
...
CREATE OR REPLACE TRIGGER trigger_update_updated_at_account
    BEFORE UPDATE
    ON account
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

CREATE OR REPLACE TRIGGER trigger_update_updated_at_appointment
    BEFORE UPDATE
    ON appointment
    FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();
